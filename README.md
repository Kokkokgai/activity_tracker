...
- **两种角色**：15 位**参与者**可记录打卡、上排行榜；**师父**是**围观者**——只看排行榜和大家的进度记录，不记录、不参与排名。
- **主页**：两个圆盘（总进度 / 剩余天数）+ 按「闻思 / 修持 / 艺术 / 身心 / 福德」分组的项目卡片，每张右下角有「记录 / 打卡」按钮。
- **记录**：填日期、备注；可**拍照 / 选图**作为证明（自动压缩后存数据库），或附**视频 / 链接**。
- **看自己的记录**：项目弹窗下方列出历史记录，**点一条可展开**看大图与完整备注。
- **每周打卡**（每周静坐 / 有氧 / 重训）：同一周只能记录一次，防止一次刷满。
- **按时长计**（弘法会 / 讲座 / 读书会）：每次记录选 0.5～3 小时，**累计满 10 小时**得 1 分，不看次数。
- **计分表**：每个项目的进度、是否得分，底部合计。
- **排行榜**：按总分排名；同分时，**越早达到该分数**的人排前面。**点任意一位**可查看 TA 每个项目的进度与记录（备注、照片对组内公开）。
